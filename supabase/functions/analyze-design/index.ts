...

{
  ...
  "prompt": {
    "site_theme": "一段100-200字的网站主题与用途描述，分析这个网站是做什么的、目标用户是谁、核心功能是什么。例如：'这是一个面向年轻消费者的潮流电商平台，主打限量球鞋和街头服饰，核心功能包括商品展示、抢购倒计时、用户社区等。'",
    "text": "一段完整的设计提示词（800-1500字），可以直接用于 AI 生成同风格页面。提示词必须包含两大部分：【第一部分：网站主题与定位】描述这个网站是做什么的、服务什么行业/场景、目标用户群体、核心功能模块。【第二部分：UI 设计风格】页面类型、整体视觉风格、颜色方案（含具体HEX值）、字体建议、圆角/阴影/间距等视觉 token、布局结构、组件风格、动画建议等。两部分结合起来，让 AI 不仅能复刻视觉风格，还能理解网站的业务定位和内容方向。用反引号标注关键CSS值和技术参数。",
    "version": "v2"
  }
}
